...
def runb(type_mode: str, date: str) -> None:
    data = get_data(type_mode, date)
    positions = get_list_of_positions(data)
    position_graph = make_position_graph(positions)
    print(f"Position graph: {position_graph}")
...
def is_valid_rectangle(p1, p2, valid_tiles):
    """
...
def find_largest_valid_rectangle(red_tiles, valid_tiles):
    """
    Find the largest rectangle with:
    - Two red tiles as opposite corners
    - All tiles in rectangle are valid (red or green)
    
    Returns (area, corner1, corner2) for the best rectangle.
    """
    best_area = 0
    best_corners = None
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if is_valid_rectangle(p1, p2, valid_tiles):
                best_area = area
                best_corners = (p1, p2)
...
def visualize_rectangle(red_tiles, valid_tiles, corner1, corner2):
    """Visualize the polygon with a rectangle highlighted."""
    red_set = set(red_tiles)
    ...
    for y in range(min_y, max_y + 1):
        row = ""
        for x in range(min_x, max_x + 1):
            in_rect = rect_min_x <= x <= rect_max_x and rect_min_y <= y <= rect_max_y
            if in_rect:
                row += "O"
            elif (x, y) in red_set:
                row += "#"
            elif (x, y) in valid_tiles:
                row += "X"
            else:
                row += "."
        print(row)

def runb_v2(type_mode: str, date: str) -> None:
    data = get_data(type_mode, date)
    red_tiles = get_list_of_positions(data)
    graph = build_red_tile_graph(red_tiles)
    
    print("Red tiles (ordered):", red_tiles)
    print("\nConnectivity graph:")
    for tile, (prev, next) in graph.items():
        print(f"  {tile} -> prev: {prev}, next: {next}")
    
    boundary = get_polygon_boundary(red_tiles)
    print(f"\nBoundary size: {len(boundary)} tiles")

    valid_tiles = get_all_valid_tiles(red_tiles)
    
    print("Polygon visualization:")
    #visualize_polygon(red_tiles, valid_tiles)
    print(f"\nTotal valid tiles: {len(valid_tiles)}")
    print(f"Red tiles: {len(red_tiles)}")
    print(f"Green tiles: {len(valid_tiles) - len(red_tiles)}")
    area, corners = find_largest_valid_rectangle(red_tiles, valid_tiles)
    print(f"Largest valid rectangle area: {area}")
    #print(f"Corners: {corners}")
    #print("\nVisualization:")
    #visualize_rectangle(red_tiles, valid_tiles, *corners)
